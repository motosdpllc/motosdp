      ...
      <div className="max-w-4xl mx-auto p-6">
        ...
        {cotsCli.length > 0 && (
          <div className="bg-white rounded-lg shadow p-6 mb-6">
            ...
            <div className="space-y-2">{cotsCli.map((c: any) => (
              <div key={c.id} className="border rounded p-4 flex justify-between items-center hover:bg-gray-50">
                <div><p className="font-bold">{c.nro}</p><p className="text-sm text-gray-600">{fmtDate(c.fecha)} · {c.cotizacion_items?.length || 0} ítems</p></div>
        ...
        <div className="bg-white rounded-lg shadow p-6">
          ...
          {loadingDetalle ? <p className="text-gray-500">Cargando...</p> : itemsCli.length === 0 ? <p className="text-gray-500">Sin ítems registrados</p> : (
            <div className="space-y-2">{itemsCli.map((x: any) => (
              <div key={x.id} className="border rounded p-4 flex justify-between items-center hover:bg-gray-50">
                <div><p className="font-bold">{x.producto} <span className="font-mono text-blue-600">{x.codigo}</span></p><p className="text-sm text-gray-600">{x.nro_orden ? 'Orden: ' + x.nro_orden + ' · ' : ''}{fmtDate(x.fecha_compra)}</p></div>
                <div className="text-right"><p className="font-bold text-green-600">{x.precio_venta ? fmt(x.precio_venta) : fmt(x.costo_total)}</p><p className="text-sm text-gray-600">{x.ubicacion}</p></div>
              </div>
            ))}</div>
          )}
        </div>
  ...
  if (vista === 'form') {
    return (
      <div className="max-w-2xl mx-auto p-6">
        <button type="button" onClick={() => { setForm(EMPTY); setEditId(null); setVista('agenda') }} className="mb-6 px-4 py-2 bg-gray-500 text-white rounded hover:bg-gray-600">← Volver</button>
        
        <div className="bg-white rounded-lg shadow p-6">
          <h1 className="text-2xl font-bold mb-6">{editId ? 'Editar cliente' : 'Nuevo cliente'}</h1>
          
          <div className="space-y-4">
            <div><label className="block text-sm font-bold mb-1">Nombre *</label><input type="text" value={form.nombre} onChange={(e) => setForm(p => ({ ...p, nombre: e.target.value }))} className="w-full border rounded px-3 py-2" /></div>
            <div><label className="block text-sm font-bold mb-1">Teléfono</label><input type="text" value={form.telefono} onChange={(e) => setForm(p => ({ ...p, telefono: e.target.value }))} className="w-full border rounded px-3 py-2" /></div>
            <div><label className="block text-sm font-bold mb-1">Dirección</label><input type="text" value={form.direccion} onChange={(e) => setForm(p => ({ ...p, direccion: e.target.value }))} className="w-full border rounded px-3 py-2" /></div>
            <div><label className="block text-sm font-bold mb-1">Código postal</label><input type="text" value={form.codigo_postal} onChange={(e) => setForm(p => ({ ...p, codigo_postal: e.target.value }))} className="w-full border rounded px-3 py-2" /></div>
            <div><label className="block text-sm font-bold mb-1">Provincia</label><select value={form.provincia} onChange={(e) => setForm(p => ({ ...p, provincia: e.target.value }))} className="w-full border rounded px-3 py-2"><option value="">— opcional —</option>{PROVINCIAS.map(p => (<option key={p} value={p}>{p}</option>))}</select></div>
            <div><label className="block text-sm font-bold mb-1">Notas</label><textarea value={form.notas} onChange={(e) => setForm(p => ({ ...p, notas: e.target.value }))} className="w-full border rounded px-3 py-2 h-20" /></div>
          </div>

          <div className="flex gap-3 mt-6">
            <button type="button" onClick={guardar} className="flex-1 px-6 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 font-bold">{editId ? 'Actualizar' : 'Guardar cliente'}</button>
            <button type="button" onClick={() => { setForm(EMPTY); setEditId(null); setVista('agenda') }} className="px-6 py-2 bg-gray-500 text-white rounded hover:bg-gray-600">Cancelar</button>
          </div>
        </div>
      </div>
    )
  ...
  return (
    <div className="max-w-6xl mx-auto p-6">
      <div className="flex justify-between items-center mb-6">
        ...
        <button type="button" onClick={() => { setForm(EMPTY); setEditId(null); setVista('form') }} className="px-6 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 font-bold">+ Nuevo cliente</button>
      </div>

      <div className="relative mb-6">
        <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar por nombre, teléfono o provincia..." className="w-full border rounded px-4 py-2 pl-4" />
        {search && <button type="button" onClick={() => setSearch('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600"><X size={20} /></button>}
      </div>

      {loading ? <p className="text-gray-500 text-center py-8">Cargando...</p> : filtered.length === 0 ? <p className="text-gray-500 text-center py-8">{search ? 'No encontrado' : 'No hay clientes aún'}</p> : search ? (
        <div className="bg-white rounded-lg shadow overflow-hidden"><div className="space-y-1">{filtered.map((c: any) => (
          <button key={c.id} type="button" onClick={() => verDetalle(c)} className="w-full text-left px-6 py-4 hover:bg-gray-50 border-b last:border-b-0 flex items-center justify-between"><div className="flex items-center gap-4 flex-1"><div className={`${colorCli(c.nombre)} w-10 h-10 rounded-full flex items-center justify-center text-white font-bold text-sm`}>{iniciales(c.nombre)}</div><div className="flex-1"><p className="font-bold">{c.nombre}</p><p className="text-sm text-gray-600">{c.telefono ? c.telefono + ' · ' : ''}{c.provincia || ''}</p></div></div></button>
        ))}</div></div>
      ) : (
        <div className="space-y-6">{Object.entries(agrupados).sort(([a], [b]) => a.localeCompare(b)).map(([letra, clis]) => (
          <div key={letra}>
            <h2 className="text-lg font-bold text-gray-700 mb-3">{letra}</h2>
            <div className="bg-white rounded-lg shadow overflow-hidden">
              <div className="space-y-1">{clis.map((c: any) => (
                <button key={c.id} type="button" onClick={() => verDetalle(c)} className="w-full text-left px-6 py-4 hover:bg-gray-50 border-b last:border-b-0 flex items-center justify-between group">
                  <div className="flex items-center gap-4 flex-1">
                    <div className={`${colorCli(c.nombre)} w-10 h-10 rounded-full flex items-center justify-center text-white font-bold text-sm`}>{iniciales(c.nombre)}</div>
                    <div className="flex-1"><p className="font-bold">{c.nombre}</p><p className="text-sm text-gray-600">{c.telefono || c.provincia || ''}</p></div>
                  </div>
                  {c.telefono && <a href={`https://wa.me/${c.telefono}`} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className="p-2 rounded-full hover:bg-green-100 text-gray-400 hover:text-green-600 transition-all flex-shrink-0"><Phone size={20} /></a>}
                </button>
              ))}</div>
            </div>
          </div>
