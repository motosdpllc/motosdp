...
export default function CotizacionesPage() {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      {loading ? (
        <p className="text-gray-500 no-print">Cargando...</p>
      ) : vista === 'lista' ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 items-start">
          <div className={`${itemActivoIndex !== null ? 'lg:col-span-3' : 'lg:col-span-4'} space-y-6 transition-all`}>
            ...
            <form onSubmit={guardar} className="space-y-6 bg-white p-6 rounded-xl shadow no-print">
                          ...
                          <td className="p-1">
                            <input 
                              ...
                              className={`w-full p-1 text-center border rounded font-bold transition-colors ${
                                alertaActiva 
                                  ? 'bg-red-600 text-white border-red-700 font-black shadow-inner' 
            ...
            <div className="space-y-8">
              ...
              {/* COTIZACIÓN CLIENTE */}
              <div className="bg-white p-6 rounded-xl shadow border border-gray-200 space-y-4 page-break">
                <div className="border-b pb-4 flex justify-between items-center">
                  <div>
                    <h2 className="text-xl font-bold text-blue-800">PRESUPUESTO DE REPUESTOS</h2>
                    <p className="text-xs text-gray-500">Referencia: <strong>{f.nro}</strong> &bull; Fecha: {f.fecha}</p>
                  </div>
                ...
                <table className="w-full text-xs text-left">
                  <thead>
                    <tr className="border-b border-gray-300 text-gray-500 font-bold bg-gray-50">
                      ...
                      <th className="p-2 w-40">Código</th>
                  ...
                  <tbody>
                    {itemsActivos.map((i, idx) => (
                      <tr key={idx} className="border-b border-gray-100">
                        ...
                        <td className="p-2 text-gray-600 uppercase text-[11px]">{i.descripcion || 'Repuesto'}</td>
                        <td className="p-2 text-right text-gray-600">${parseFloat(i.precio_venta || 0).toFixed(2)}</td>
                        <td className="p-2 text-right font-semibold text-gray-900">${((Number(i.cantidad) || 1) * parseFloat(i.precio_venta || 0)).toFixed(2)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <div className="space-y-6">
                <h2 className="text-lg font-black text-gray-800 border-b pb-1 uppercase">📋 Órdenes de Compra por Distribuidor</h2>

                {deBasoli.length > 0 && (
                  <div className="border border-gray-200 rounded-lg p-4 bg-white page-break">
                    <h3 className="font-bold text-gray-900 text-xs uppercase mb-2 border-b pb-1">Distribuidor: BÁSOLI</h3>
                    <table className="w-full text-xs text-left">
                      <thead>
                ...
                {dePartzilla.length > 0 && (
                  <div className="border border-gray-200 rounded-lg p-4 bg-white page-break">
                    <h3 className="font-bold text-gray-900 text-xs uppercase mb-2 border-b pb-1">Distribuidor: PARTZILLA</h3>
                    <table className="w-full text-xs text-left">
                      <thead>
                        <tr className="text-gray-500 border-b font-bold"><th className="py-1 w-12 text-center">Cant</th><th className="py-1 w-40">Código</th><th className="py-1">Descripción</th><th className="py-1 text-right w-24">Costo Neto</th><th className="py-1 text-right w-24">Costo + 1.11</th></tr>
                      </thead>
                ...
                {deOtra.length > 0 && (
                  <div className="border border-purple-200 rounded-lg p-4 bg-white page-break">
                    <h3 className="font-bold text-purple-900 text-xs uppercase mb-2 border-b pb-1">Distribuidor: Otros Externos</h3>
                    <table className="w-full text-xs text-left">
                      <thead>
                ...
                {itemsPendientes.length > 0 && (
                  <div className="border border-red-200 rounded-lg p-4 bg-red-50/30 no-print">
                    <h3 className="font-bold text-red-800 text-sm uppercase mb-1">⚠️ Pendientes de Cotizar/Ubicar:</h3>
                    <div className="bg-white border border-red-100 p-2 rounded font-mono text-xs text-red-600 font-bold">
                      {itemsPendientes.map(i => i.codigo).join(', ')}
                    </div>
                  </div>
                )}
              </div>
          ...
          {itemActivoIndex !== null && (
            <div className="bg-white p-4 rounded-xl shadow border border-gray-200 space-y-4 lg:col-span-1 sticky top-6 no-print">
              <div className="flex justify-between items-center border-b pb-2">
                <h3 className="font-bold text-sm text-blue-700">⚙️ Detalles Línea #{itemActivoIndex + 1}</h3>
                <button type="button" onClick={() => setItemActivoIndex(null)} className="text-gray-400 hover:text-gray-600 text-sm font-bold">✖</button>
              </div>
