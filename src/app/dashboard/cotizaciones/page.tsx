...
export default function CotizacionesPage() {
  ...
  const actualizarCeldaItem = (index: number, campo: string, valor: any) => {
    ...
    if (['peso', 'basoli', 'partzilla', 'otra', 'precio_venta'].includes(campo)) {
      copia[index] = { ...copia[index], [campo]: valor === '' ? 0 : parseFloat(valor) || 0 }
    } else if (campo === 'cantidad') {
      copia[index] = { ...copia[index], [campo]: valor === '' ? 0 : parseInt(valor) || 0 }
    } else {
  ...
  const obtenerCostoConRecargo = (item: any) => {
    let original = 0
    if (item.proveedor_elegido === 'basoli') original = Number(item.basoli) || 0
    if (item.proveedor_elegido === 'partzilla') original = Number(item.partzilla) || 0
    if (item.proveedor_elegido === 'otra') original = Number(item.otra) || 0
    return parseFloat((original * MULTIPLICADOR).toFixed(2))
  }

  // Alerta estricta: si hay código/desc, cantidad > 0 y la venta es menor al costo inflado
  const verificarAlertaVenta = (item: any) => {
    if (!item.codigo && !item.descripcion) return false
    if ((Number(item.cantidad) || 0) <= 0) return false
    const costoConRecargo = obtenerCostoConRecargo(item)
    const venta = Number(item.precio_venta) || 0
    return venta > 0 && venta < costoConRecargo
  }
  ...
  const editarCot = (cot: any) => {
    ...
    const itemsCargados = (cot.cotizacion_items || []).map((item: any) => ({
      ...
      basoli: parseFloat(item.basoli || '0'),
      partzilla: parseFloat(item.partzilla || '0'),
      otra: parseFloat(item.otra || '0'),
  ...
  // Generar mensaje formateado para enviar al cliente por WhatsApp
  const mandarWhatsApp = () => {
    const validos = cotItems.filter(i => (i.codigo.trim() !== '' || i.descripcion.trim() !== '') && i.cantidad > 0)
    let msg = `*Presupuesto ${f.nro}* \n`
    msg += `Cliente: ${f.cliente_nombre}\n`
    if (f.vin) msg += `VIN/Chasis: ${f.vin}\n`
    msg += `-----------------------------------\n`
    
    let total = 0
    validos.forEach(i => {
      const subtotal = (Number(i.cantidad) || 1) * (Number(i.precio_venta) || 0)
      total += subtotal
      msg += `${i.cantidad}x  ${i.codigo} - ${i.descripcion || 'Repuesto'} -> *$${subtotal.toFixed(2)}*\n`
    })
    
    msg += `-----------------------------------\n`
    msg += `*TOTAL FINAL: $${total.toFixed(2)}*`
  ...
  const itemsActivos = cotItems.filter(i => (i.codigo.trim() !== '' || i.descripcion.trim() !== '') && (Number(i.cantidad) || 0) > 0)
  const itemsPendientes = cotItems.filter(i => (i.codigo.trim() !== '' || i.descripcion.trim() !== '') && (Number(i.cantidad) || 0) === 0)
  ...
  // Totalizador del presupuesto del cliente
  const totalVentaCliente = itemsActivos.reduce((acc, curr) => acc + ((Number(curr.cantidad) || 1) * (Number(curr.precio_venta) || 0)), 0)

  return (
    <div className="p-6 max-w-7xl mx-auto">
      {/* Inyección de estilos CSS de impresión para ocultar la basura estructural */}
      <style dangerouslySetInnerHTML={{__html: `
        @media print {
          body { background: white; color: black; }
          nav, sidebar, button, form, .print\\:hidden, input, select, textarea { display: none !important; }
      ...
      {loading ? (
        <p className="text-gray-500 print:hidden">Cargando...</p>
      ) : vista === 'lista' ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 items-start">
          <div className={`${itemActivoIndex !== null ? 'lg:col-span-3' : 'lg:col-span-4'} space-y-6 transition-all`}>
            ...
            {/* FORMULARIO DE EDICIÓN: OCULTO EN IMPRESIÓN */}
            <form onSubmit={guardar} className="space-y-6 bg-white p-6 rounded-xl shadow print:hidden">
                ...
                <table className="min-w-full divide-y divide-gray-200 text-xs">
                  ...
                  <tbody className="divide-y divide-gray-200 bg-white">
                    {cotItems.map((item, index) => {
                      const costoConRecargo = obtenerCostoConRecargo(item)
                      const alertaActiva = verificarAlertaVenta(item)
                        ...
                        <tr key={index} className={`hover:bg-gray-50 transition-colors ${item.cantidad === 0 && item.codigo ? 'bg-red-50/60' : ''} ${estaSeleccionado ? 'bg-blue-50/70 border-2 border-blue-400' : ''}`}>
                          ...
                          <td className="p-1">
                            <input 
                              ...
                              onClick={() => setItemActivoIndex(index)}
                              className="w-full p-1 text-center border rounded bg-purple-50 font-medium text-purple-800" 
                            />
                          </td>
                          <td className="p-1">
                          ...
                          <td className="p-1">
                            <input 
                              ...
                              onChange={e => actualizarCeldaItem(index, 'precio_venta', e.target.value)} 
                              className={`w-full p-1 text-center border rounded font-bold transition-colors ${
                                alertaActiva 
                                  ? 'bg-red-600 text-white border-red-700 font-black animate-pulse shadow-inner' 
            ...
            {/* SECCIÓN DOCUMENTOS: OPTIMIZADO PARA PANTALLA E IMPRESIÓN */}
            <div className="space-y-8">
              
              {/* BOTONERA EXCLUSIVA DE CONTROL */}
              <div className="bg-white p-4 rounded-xl shadow border flex space-x-3 print:hidden">
                <button onClick={() => window.print()} className="bg-gray-800 text-white text-xs px-4 py-2 rounded-lg font-bold hover:bg-gray-900 flex items-center">
                  🖨️ Imprimir Notas de Pedido / Presupuestos
                </button>
                <button onClick={mandarWhatsApp} className="bg-green-600 text-white text-xs px-4 py-2 rounded-lg font-bold hover:bg-green-700 flex items-center">
                  💬 Enviar Presupuesto por WhatsApp Web
                </button>
              </div>

              {/* DOCUMENTO 1: COTIZACIÓN LIMPIA PARA EL CLIENTE */}
              <div className="bg-white p-6 rounded-xl shadow border border-gray-200 space-y-4">
                <div className="border-b pb-4 flex justify-between items-center">
                  <div>
                    <h2 className="text-xl font-bold text-blue-800">PRESUPUESTO DE REPUESTOS</h2>
                    <p className="text-xs text-gray-500">Documento de Referencia: <strong>{f.nro}</strong> &bull; Fecha: {f.fecha}</p>
                  </div>
                  <div className="text-right">
                    <span className="text-xs font-bold uppercase tracking-wider text-gray-400 block">Cliente</span>
                    <span className="font-bold text-gray-800">{f.cliente_nombre || 'Sin Asignar'}</span>
                    {f.vin && <p className="text-xs text-gray-500 mt-0.5">VIN: {f.vin}</p>}
                  </div>
                </div>

                <table className="w-full text-xs text-left">
                  <thead>
                    <tr className="border-b border-gray-300 text-gray-500 font-bold bg-gray-50">
                      <th className="p-2 w-12 text-center">Cant</th>
                      <th className="p-2 w-40">Código Partes</th>
                      <th className="p-2">Descripción</th>
                      <th className="p-2 text-right w-32">Unitario</th>
                      <th className="p-2 text-right w-32">Subtotal</th>
                    </tr>
                  </thead>
                  <tbody>
                    {itemsActivos.map((i, idx) => (
                      <tr key={idx} className="border-b border-gray-100 hover:bg-gray-50/50">
                        <td className="p-2 text-center font-bold text-gray-800">{i.cantidad}</td>
                        <td className="p-2 font-mono font-bold text-gray-700">{i.codigo}</td>
                        <td className="p-2 text-gray-600 uppercase text-[11px]">{i.descripcion || 'Repuesto original'}</td>
                        <td className="p-2 text-right text-gray-600">${parseFloat(i.precio_venta || 0).toFixed(2)}</td>
                        <td className="p-2 text-right font-semibold text-gray-900">${(Number(i.cantidad) * parseFloat(i.precio_venta || 0)).toFixed(2)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                ...
                <div className="flex justify-end pt-4">
                  <div className="bg-gray-50 border p-3 rounded-lg text-right w-64">
                    <span className="text-xs font-bold text-gray-500 block uppercase">Total Presupuestado</span>
                    <span className="text-xl font-black text-green-700">${totalVentaCliente.toFixed(2)}</span>
                  </div>
                </div>
              </div>
              ...
              {/* DOCUMENTO 2: HOJAS DE PEDIDO INTERNAS PARA PROVEEDORES */}
              <div className="space-y-6">
                <h2 className="text-lg font-black text-gray-800 tracking-wide border-b pb-1 uppercase print:visible">📋 Distribución de Pedidos por Proveedor</h2>

                {deBasoli.length > 0 && (
                  <div className="border border-gray-200 rounded-lg p-4 bg-white page-break">
                    <h3 className="font-bold text-gray-900 text-xs uppercase mb-2 border-b pb-1 tracking-wider">Orden de Compra: BÁSOLI</h3>
                    <table className="w-full text-xs text-left">
                      <thead>
                        <tr className="text-gray-500 border-b font-bold"><th className="py-1 w-12 text-center">Cant</th><th className="py-1 w-40">Código</th><th className="py-1">Descripción</th><th className="py-1 text-right w-24">Costo Neto</th><th className="py-1 text-right w-24">Costo * 1.11</th></tr>
                      </thead>
                      <tbody>
                        {deBasoli.map((i, idx) => (
                          <tr key={idx} className="border-b border-gray-100"><td className="py-1 text-center font-bold">{i.cantidad}</td><td className="py-1 font-mono">{i.codigo}</td><td className="py-1 text-gray-600">{i.descripcion}</td><td className="py-1 text-right">${parseFloat(i.basoli).toFixed(2)}</td><td className="py-1 text-right font-bold">${(parseFloat(i.basoli) * MULTIPLICADOR).toFixed(2)}</td></tr>
                        ))}
                      </tbody>
                    </table>
                  </div>
                ...
                {dePartzilla.length > 0 && (
                  <div className="border border-gray-200 rounded-lg p-4 bg-white page-break">
                    <h3 className="font-bold text-gray-900 text-xs uppercase mb-2 border-b pb-1 tracking-wider">Orden de Compra: PARTZILLA</h3>
                    <table className="w-full text-xs text-left">
                      <thead>
                        <tr className="text-gray-500 border-b font-bold"><th className="py-1 w-12 text-center">Cant</th><th className="py-1 w-40">Código</th><th className="py-1">Descripción</th><th className="py-1 text-right w-24">Costo Neto</th><th className="py-1 text-right w-24">Costo * 1.11</th></tr>
                      </thead>
                      <tbody>
                        {dePartzilla.map((i, idx) => (
                          <tr key={idx} className="border-b border-gray-100"><td className="py-1 text-center font-bold">{i.cantidad}</td><td className="py-1 font-mono">{i.codigo}</td><td className="py-1 text-gray-600">{i.descripcion}</td><td className="py-1 text-right">${parseFloat(i.partzilla).toFixed(2)}</td><td className="py-1 text-right font-bold">${(parseFloat(i.partzilla) * MULTIPLICADOR).toFixed(2)}</td></tr>
                        ))}
                      </tbody>
                    </table>
                  </div>
                ...
                {deOtra.length > 0 && (
                  <div className="border border-purple-200 rounded-lg p-4 bg-white page-break">
                    <h3 className="font-bold text-purple-900 text-xs uppercase mb-2 border-b pb-1 tracking-wider">Orden de Compra: Web Externas / Otros</h3>
                    <table className="w-full text-xs text-left">
                      <thead>
                        <tr className="text-gray-500 border-b font-bold">
                      ...
                      <tbody>
                        {deOtra.map((i, idx) => (
                          <tr key={idx} className="border-b border-gray-100">
                            ...
                            <td className="py-1 font-bold text-purple-700 uppercase">{i.proveedor_otro_nombre || 'Web Externa'}</td>
                ...
                {itemsPendientes.length > 0 && (
                  <div className="border border-red-200 rounded-lg p-4 bg-red-50/30 print:hidden">
          ...
          {itemActivoIndex !== null && (
            <div className="bg-white p-4 rounded-xl shadow border border-gray-200 space-y-4 lg:col-span-1 sticky top-6 print:hidden">
              ...
              <div className="bg-gray-50 p-2.5 rounded border">
                ...
                <input 
                  ...
                  value={cotItems[itemActivoIndex]?.peso || 0} 
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 items-start">
          <div className={`${itemActivoIndex !== null ? 'lg:col-span-3' : 'lg:col-span-4'} space-y-6 transition-all`}>
            ...
            <form onSubmit={guardar} className="space-y-6 bg-white p-6 rounded-xl shadow print:hidden">
              ...
              <div className="flex justify-end space-x-3 pt-4 border-t">
                <button type="button" onClick={() => setVista('lista')} className    const { data: clis } = await supabase.from('clientes').select('*').order('nombre')
    if (cots) setCotizaciones(cots)
    if (clis) setClientes(clis)
    setLoading(false)
  }
