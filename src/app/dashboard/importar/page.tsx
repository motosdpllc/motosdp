...
export default function ImportarPage() {
  ...
  const procesarArchivo = async (file: File) => {
    ...
    try {
      ...
      // Llamada a tu API de backend (asegúrate de que esta API exista y use la IA)
      // Este endpoint debería tomar el base64 y devolver el JSON esperado
      const res = await fetch('/api/parse-invoice', { // <-- ASUME QUE TIENES UN /api/parse-invoice
        method: 'POST',
      ...
      // Mapear los ítems de la respuesta de la IA al formato ItemImportado
  ...
  // Lógica para asignar cliente global a todos los ítems seleccionados
    ...
    try {
      for (const item of seleccionados) {
        await supabase.from('items').insert({
          ...
          oem: item.oem || null,
          importe: item.importe_unitario, // importe es el costo del producto
          costo_envio: item.costo_envio_unitario,
          taxes: item.taxes_unitario,
          reembolsos: item.reembolsos_unitario,
          costo_total: item.costo_total_unitario, // Guardar el costo total calculado
          cantidad: item.cantidad, // Guardar la cantidad
          nro_orden: nroOrden || null,
          pagina: proveedor || null, // Usar proveedor como página de compra
          ...
          // Los campos de código, marca, modelo, etc., se pueden inferir o dejar vacíos si la IA no los extrae.
        })
      }
    ...
    <div className="min-h-screen bg-gray-50 p-6 flex flex-col items-center justify-center">
      <div className="bg-white rounded-lg shadow-xl p-8 text-center max-w-md">
        ...
        <div className="flex gap-4">
          ...
          <button onClick={() => { setPaso('upload'); setItems([]) }} className="flex-1 btn bg-gray-200 text-gray-800 hover:bg-gray-300">Importar otro</button>
        </div>
      </div>
    </div>
  )
  ...
  if (paso === 'revisar') return (
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-6xl mx-auto bg-white rounded-lg shadow-xl p-8">
        <button type="button" onClick={() => setPaso('upload')} className="mb-6 px-4 py-2 bg-gray-500 text-white rounded-lg hover:bg-gray-600 font-semibold">
          ← Volver a subir
        </button>
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6 flex flex-col items-center justify-center">
      <div className="bg-white rounded-lg shadow-xl p-8 text-center max-w-md">
        <h1 className="text-3xl font-bold mb-4">Importar factura de compra</h1>
        <p className="text-gray-600 mb-6">Subí la factura del proveedor — la IA extrae los ítems automáticamente.</p>
