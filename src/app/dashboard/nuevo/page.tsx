...
interface ItemForm {
  ...
  // Para el estado "Pendiente de comprar"
...
function NuevoForm() {
  ...
  useEffect(() => {
    if (!editId && f.tracking_compra && f.tracking_compra.trim() !== '') {
      // Si hay tracking, asumimos que está en tránsito
      setF(prev => ({ ...prev, ubicacion: 'En tránsito' }));
    } else if (!editId && f.ubicacion === 'En tránsito') {
      // Si el tracking se borra, y no está editando, vuelve a proveedor
  ...
  const guardar = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload: any = {
        ...
        pendiente_compra: tipoCarga === 'Venta' && f.ubicacion === 'Proveedor' // Si es venta y está en proveedor, está pendiente
      }
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-6xl mx-auto bg-white rounded-lg shadow-xl p-8">
        ...
        <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-4 rounded-lg flex justify-center gap-4 mb-8">
          <button onClick={() => { setTipoCarga('Stock'); setF(p => ({ ...p, destino: 'Stock EEUU', ubicacion: 'Proveedor', cliente_id: null, cliente_nombre: null })) }} className={`px-6 py-2 rounded-lg font-medium text-sm transition-all ${tipoCarga === 'Stock' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-600 hover:text-gray-900'}`}>
            📦 Carga para Stock Directo
          </button>
          <button onClick={() => { setTipoCarga('Venta'); setF(p => ({ ...p, destino: 'Vendido', ubicacion: 'Proveedor', estado_pago: 'Debe' })) }} className={`px-6 py-2 rounded-lg font-medium text-sm transition-all ${tipoCarga === 'Venta' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-600 hover:text-gray-900'}`}>
        ...
        <form onSubmit={guardar}>
          ...
          {!editId && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
              ...
              <a href="/dashboard/nuevo/importar" className="px-6 py-2 bg-orange-600 text-white rounded-lg hover:bg-orange-700 font-bold text-center">
          ...
          <div className="bg-gray-50 p-6 rounded-lg mb-8 border border-gray-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input type="text" value={f.codigo} onChange={e => setF(p => ({ ...p, codigo: e.target.value }))} className="w-full border rounded px-3 py-2 bg-gray-100" placeholder="Código único" required />
              </div>
              <div>
            ...
            <div className="bg-yellow-50 p-6 rounded-lg mb-8 border border-yellow-200">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="relative">
                  ...
                  <input type="text" value={cliSearch} onChange={e => { setCliSearch(e.target.value); setShowCliDrop(true); if (!e.target.value) setF(p => ({ ...p, cliente_id: '', cliente_nombre: '' })) }} onFocus={() => { if (cliSearch) setShowCliDrop(true) }} className="w-full border rounded px-3 py-2" placeholder="Buscar cliente..." />
          ...
          <div className="flex justify-end gap-4 mt-8">
            ...
            <button type="submit" disabled={saving} className="px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 font-bold disabled:opacity-50">
